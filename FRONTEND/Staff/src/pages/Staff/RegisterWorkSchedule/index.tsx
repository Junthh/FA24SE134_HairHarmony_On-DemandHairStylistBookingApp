                    ...
                    <StyledTableRow key={i}>
                      ...
                      <StyledTableCell align="center">
                        ...
                        row.dayOffs.filter((value) => value.isApprove && value.type === 'P')
                          ...
                          ? 'Nghỉ phép'
                          : row.dayOffs &&
                            row.dayOffs.filter((value) => value.isApprove && value.type === 'KP')
                              .length > 0
                          ? 'Nghỉ không phép'
                          : row.dayOffs &&
                            row.dayOffs.filter(
                              (value) => value.isApprove === null && value.type === 'P',
                            ).length > 0
                          ? 'Đang duyệt nghỉ phép'
                          : row.dayOffs &&
                            row.dayOffs.filter(
                              (value) => value.isApprove === null && value.type === 'KP',
                            ).length > 0
                          ? 'Đang duyệt nghỉ không phép'
                          : 'Làm việc'}
                      </StyledTableCell>
                      <StyledTableCell
                        align="center"
