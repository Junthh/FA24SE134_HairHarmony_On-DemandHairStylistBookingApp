...
export default function Category() {
  ...
  const columns = [
    ...
    columnHelper.accessor('color', {
      ...
      cell: (info) => (
        <Box borderRadius={'4px'} width={24} height={24} bgcolor={info.renderValue()}></Box>
      ),
      footer: (info) => info.column.id,
      enableSorting: false,
    }),
  ...
  const fetchData = async () => {
    try {
      ...
      const resService: ListServiceSuccess | DataServiceError | any = await categoryService.list(
        params,
      );
  ...
  const handleDelete = async (id: string) => {
    try {
      dispatch(setLoading(true));
      const resService: DataServiceSuccess | DataServiceError | any = await categoryService.delete(
        id,
      );
      if (resService.success) {
        await fetchData();
