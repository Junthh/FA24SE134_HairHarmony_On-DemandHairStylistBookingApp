...
import { EcoStoriesCardStyled } from 'pages/Users/EcoStories/styles';
  ...
  return (
    <NewestEcoStoriesStyled color={color} background={background}>
      ...
      <Box className="content_body">
        ...
        <Box className="content_body-right">
          {posts.newest.slice(1).map((p) => {
            return (
              <EcoStoriesCardStyled
                color={color}
                background={background}
                key={p.id}
                onClick={() => handleGetDetail(p)}
              >
                {!isMobile && (
                  <Box className="image-national">
                    <img className="image-fit-center" src={countriesFlag[p?.country]} alt="" />
                  </Box>
                )}
                <Box className="tag-container">
                  <Box className="tag">
                    {p?.category?.name && (
                      <TagStyledWrapper catesColor={p?.category?.color}>
                        <TagStyled catesColor={p?.category?.color}>{p?.category?.name}</TagStyled>
                      </TagStyledWrapper>
                    )}
                    {(p.tags?.split('&#&') || []).map((a, index) => (
                      <TagTextStyled tagColor={tagColor} key={index}>
                        {a}
                      </TagTextStyled>
                    ))}
                  </Box>
                  {/* {isMobile && (p.tags?.split('&#&')?.length || 0) > 1 ? (
                    <span className="dots">...</span>
                  ) : (
                    <></>
                  )} */}
                ...
                <Box className="item">
                  ...
                  <Typography
                    fontSize={20}
                    sx={{ width: !isMobile ? '80%' : '95%' }}
                    lineHeight={'140%'}
                    fontWeight={700}
                    color={textColor}
                    className="multiline-ellipsis item-title"
                  >
                    {p.title}
                  </Typography>
                  <Box
                  ...
                  >
                    <img
                      className="image-fit-center"
                      src={p.mediaUrl || IMAGE_ECO_STORIES1}
                      alt=""
                    />
                  </Box>
                </Box>
