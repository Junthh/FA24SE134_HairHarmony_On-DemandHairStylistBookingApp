...
export default function ArticleDetails() {
  ...
  const schema = Yup.object().shape<any>({
    ...
    categoryId: Yup.string().required('Category is require.'),
  });
  ...
  useEffect(() => {
    ...
    Promise.all([fetchWriter(), fetchProject(), fetchCategory()])
      .then(([writerOtp, projectOtp, categoryOpt]) => {
        //update
        const pId = detachParamID(location.pathname, '/admin/article/');
        if (pId && pId !== '' && pId !== 'create') {
          fetchData(pId, writerOtp, projectOtp, categoryOpt);
        }
      })
      .catch((err) => {
  ...
  const fetchData = async (
    id: string,
    writerOtp: Array<any> = [],
    projectOtp: Array<any> = [],
    categoryOtp: Array<any> = [],
  ) => {
    try {
      dispatch(setLoading(true));
      const resService: DataServiceSuccess | DataServiceError | any = await articleService.find(id);
      const data: ArticleForm = resService.data;
      if (resService.success) {
        setForm(data, writerOtp, projectOtp, categoryOtp);
      }
    } catch (error) {
      console.error('Error', error);
      showToast('error', 'Load data failed.');
  ...
  const handlePublish = handleSubmit(async (formData: ArticleForm) => {
    try {
      dispatch(setLoading(true));
      if (file) {
      ...
      if (dataUpated) {
        const resService: DataServiceSuccess | DataServiceError | any = await articleService.update(
          dataUpated.id,
          formData,
        );
        if (resService.success) {
        ...
        const resService: DataServiceSuccess | DataServiceError | any = await articleService.create(
          formData,
        );
        if (resService.success) {
          handleResetFile();
  ...
  const setForm = (
    ...
    const catId = data.category && data.category.id ? data.category.id : null;
    const arrAvailableCatId = categoryOptions.length ? categoryOptions : categoryOtp;
    setValue('categoryId', checkItemIdInOptions(catId, arrAvailableCatId));
  };
    ...
    <ContainerDetailsWrap>
      ...
      <BodyContentWarp>
        <Typography variant="h4" fontWeight={700}>
          {dataUpated ? 'Update' : 'Create New'} Article
        </Typography>
